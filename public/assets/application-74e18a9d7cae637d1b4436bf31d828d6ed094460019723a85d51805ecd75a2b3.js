...
(function() {
  ...
  (function($) {
    ...
    return $.fn.addFormFields = function(options) {
      var count, settings;
      count = 1;
      settings = $.extend({
        target: null,
      ...
      this.click(function() {
        var $cloned, $removeButton, lastInput, lastInputParentDiv;
        ...
        if (lastInputParentDiv.length === 0) {
          $cloned.addClass('input-append');
          $cloned.css("display", "block");
        } else {
          lastInputParentDiv.addClass('input-append');
        }
        ...
        lastInput.removeClass(function(index, css) {
...
(function() {
  ...
  $(document).ready(function() {
    var tapasApp;
    tapasApp = (function() {
      var CommunitiesPage, CoreFilesPage, handleRequiredInputs, init, initModal, removeFormFields, setup, tapasAppError, tooltipSetup;
       ...
       */
      initModal = function(heading, body, footer, show) {
        var clear, hanldleRemoteLinks, listenForAjax, reloadWindow, t;
        if (heading == null) {
          heading = "Modal";
        }
        if (body == null) {
        ...
        listenForAjax = function(element) {
          return $(element).on('ajax:complete', reloadWindow);
        };
        hanldleRemoteLinks = function() {
          var i, len, link, remoteLinks, results;
          remoteLinks = t.find('a[data-remote]');
          results = [];
          for (i = 0, len = remoteLinks.length; i < len; i++) {
            link = remoteLinks[i];
            results.push(listenForAjax(link));
          }
          return results;
        };
        hanldleRemoteLinks();
        return t.on('hidden', clear);
      };
      removeFormFields = function() {
        var handleClick;
        handleClick = function(e) {
          var $el, $remove, removeSelector;
          $el = $(this);
          e.preventDefault();
          ...
          if ($remove.length === 0) {
            $remove = $(removeSelector).length === 1 ? $(removeSelector) : null;
          }
          if ($remove != null) {
            $el.remove();
            return $remove.empty().remove();
          } else {
        ...
        if (tapasApp.config.removeFormFields.listener) {
          return $('*[data-delete ]').on('click', handleClick);
        } else {
          return $('body').on('click', '*[data-delete]', handleClick);
        }
      };
      tapasAppError = function(message, value) {
        if (message == null) {
          message = 'Error:';
        }
        if (value == null) {
