...
2. Install [Xcode](https://developer.apple.com/xcode/) from the Mac App Store.
  a. Install the Command Line Tools package with `xcode-select --install`.
3. Install the `automake` package with your package manager.
4. Install the `openssl@3` package with your package manager.
5. Install Ruby:
  a. Try RVM's default install, `rvm install ruby-2.7.7` (or the shorthand `rvm install 2.7.7`).
  b. If the installation failed because the ruby source couldn't be compiled, provide the OpenSSL path, e.g.: `rvm install ruby-2.7.7 -C --with-openssl-dir='/opt/homebrew/bin/openssl'`.
...
5. Create a Solr core for TAPAS:
  a. Run the core generator: `bin/solr create -c tapas-core`
  b. Go to the core configuration directory: `cd server/solr/tapas-core/conf`
  c. Create a configuration file: `touch solrconfig.xml`
  d. Create a schema file: `touch schema.xml`
  e. Populate each file with the contents of the files received with the same names using the text editor of your choice. **??? Can we make this easier?**
6. Confirm that the new core has been configured correctly:
   a. In your browser, open the [Solr Admin Dashboard](http://localhost:8983/solr/#/)
   b. Click on the **Core Selector** dropdown button on the left side of the page.
   c. Verify that `tapas-core` is listed.
...
2. If you are on a fresh install, with Homebrew:
  ...
  b. Configure the root user's password with `mysql_secure_installation`
3. Set up a MySQL user for TAPAS:
  a. Log into MySQL as the root user: `sudo mysql -u root -p`
  b. Create the TAPAS user, e.g. `create user 'tapas_user'@'localhost' identified by 'changeThisPassword!';`
  c. Make sure the new user has permissions to create and edit databases: `grant all on *.* to 'tapas_user'@'localhost' with grant option;`
  d. Quit the MySQL command line interface: `\q`
...
2. Configure your environment variables:
  a. Create a plain text file named `.env`.
  b. Define the environment variables you will need, following the [syntax required by dotenv](https://github.com/bkeepers/dotenv/blob/main/README.md#usage). An example appears below.
  c. Save the `.env` file to the `tapas_rails` directory.
...
4. Install the `mysql2` gem:
  a. If you installed OpenSSL and/or MySQL with Homebrew, try: `gem install mysql2 -- --with-mysql-dir=$(brew --prefix mysql) --with-openssl-dir=$(brew --prefix openssl@3)`
  b. The installation above should complete without error. If it doesn't, check the [`mysql2` installation instructions](https://github.com/brianmario/mysql2#installing) for options that might correspond to the errors you're seeing. 
  c. *Do __not__ proceed until `mysql2` has installed successfully.*
5. Install all other gems required by TAPAS: `bundle install`.
6. Set up the TAPAS databases:
  a. Create the app databases: `rails db:create`
  b. Create tables within the databases: `rails db:migrate`
