...
describe ProjectsController do
  ...
  #
  #   it '404s for dids that do not tapas_xq' do
  ...
  # end
    ...
    describe 'POST #upsert' do
      ...
      let(:project) { Project.find_by_did params[:did] }
      ...
      it 'returns a 202 and creates community on requests with new dids.' do
        ...
        expect(project.depositor).to eq params[:depositor]
      end

      it 'returns a 202 and updates the requested community if it exists' do
        community_old = Project.new
        ...
        post :upsert, params
        expect(response.status).to eq 202
        expect(project.depositor).to eq 'System'
        expect(project.project_members).to eq ['1', '2', '3']
      end
    end
  ...
  describe 'post #create' do
    ...
    before(:all) {
      ...
      @communityCreated = Project.new(title:'New Community', depositor:@user.id.to_s, description:'This is a test community.', mass_permissions:'public')
    ...
    let(:project) {
      ...
      Project.find_by_did params[:did] }
    ...
    it 'should create a community object and go to show page' do
      ...
      expect(community.title).to eq params[:project][:title]
    ...
    it 'community title should be consistent' do
      ...
      expect(@communityCreated.title).to eq params[:project][:title]
    end
    ...
    it 'community description should be consistent' do
      ...
      expect(@communityCreated.description).to eq params[:project][:description]
    end
    ...
    it 'community mass permission should be consistent' do
      ...
      expect(@communityCreated.mass_permissions).to eq params[:project][:mass_permissions]
    end
  end
  ...
  describe 'post #update' do
    ...
    it '302s for valid requests' do
      ...
      project.depositor = user.id.to_s
      project.save!
      params = { :did=> project.did, :id=>project.pid, :project=>{ :title=>'Updated community', :mass_permissions=>'public', :description=>'Updated description'}}
