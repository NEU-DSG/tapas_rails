...
describe Project do
  ...
  it 'has many Users as members' do
    expect(project.users.count).to equal(1)
  end

  it 'has a depositor' do
    expect(project.depositor).to be(user)
  end

  describe '.create' do
    it 'sets members' do
      c = Project.create!(title: "Title", depositor: user, institutions: [institution])
    end
  end
  ...
  # it "can create the root community when it doesn't tapas_xq" do
  ...
  #   it 'populates values appropriately where they tapas_xq' do
